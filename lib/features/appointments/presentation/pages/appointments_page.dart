...
class _AppointmentsPageState extends State<AppointmentsPage> {
  ...
  Widget _buildCalendar() {
    ...
    // ⚡ Bolt: Pre-compute normalized dates for O(1) lookups during render
    // What: Extracted O(n) `.any()` checks from the grid builder into a pre-computed Set.
    // Why: `List.any()` inside `itemBuilder` scales linearly with both days and appointments, causing lag.
    // Impact: Reduces time complexity per cell from O(A) to O(1).
    final appointmentDates = _allAppointments.map((a) {
      return DateTime(a.dateTime.year, a.dateTime.month, a.dateTime.day);
    }).toSet();
