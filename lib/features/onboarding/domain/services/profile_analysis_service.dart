...
class ProfileAnalysisService {
  ...
  RelevantStandards analyzeProfile(OnboardingUserProfile profile) {
  ...
  List<Icd10Code> getRelevantIcd10Codes(OnboardingUserProfile profile) {
  ...
  List<LoincCode> getRelevantLoincCodes(OnboardingUserProfile profile) {
    final standards = analyzeProfile(profile);
  ...
  List<ClinicalGuidelineReference> getRelevantGuidelines(OnboardingUserProfile profile) {
    final standards = analyzeProfile(profile);
  ...
  List<String> getRelevantMedicationClasses(OnboardingUserProfile profile) {
    final standards = analyzeProfile(profile);
    return standards.medicationClasses;
  }

  /// Estimate local storage size for this profile
  int estimateStorageSizeMb(OnboardingUserProfile profile) {
    final standards = analyzeProfile(profile);
