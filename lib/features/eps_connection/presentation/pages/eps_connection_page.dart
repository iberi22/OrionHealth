...
import 'eps_qr_scanner_page.dart';
    ...
    return GlassmorphicCard(
      child: InkWell(
        onTap: () async {
          ...
          if (qrData != null) {
            // Process QR data to create EPSProvider
            // Example format: providerId|name|discoveryUrl|clientId
            final parts = qrData.split('|');
            if (parts.length >= 4) {
              final provider = EPSProvider(
                id: parts[0],
                name: parts[1],
                discoveryUrl: parts[2],
                clientId: parts[3],
                redirectUrl: 'com.orionhealth.app://oauth-callback',
                scopes: const ['openid', 'profile', 'email', 'patient/*.read'],
              );
              if (context.mounted) {
