...
class ProfileAnalyzer {
  ...
  RelevantStandards analyzeProfile({
    ...
    for (final cat in categories) {
      icd10Codes.addAll(CategoryIcd10.forCategory(cat));
      loincCodes.addAll(CategoryLoinc.forCategory(cat));
      medicationClasses.addAll(CategoryMedications.forCategory(cat));
    }

    return RelevantStandards(
      categories: categories,
      tier1: {MedicalContextCategory.preventive},
      tier2: categories.difference({MedicalContextCategory.preventive}),
      icd10Codes: icd10Codes,
      loincCodes: loincCodes,
      medicationClasses: medicationClasses,
      guidelineIds: _getGuidelineIds(categories),
      estimatedSizeMB: categories.length * 20, // Rough estimation
    );
  }
