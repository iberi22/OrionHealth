...
describe('agentDomain 100% agentico', () => {
  ...
  it('agentCreate crea fila + memoria + mesh', async () => {
    const rec: any = await agentCreate('record', { name: 'AgentItem' } as any, 'inst1');
    expect(rec.id).toBeTruthy();
    expect(rec.instance_id).toBe('inst1');
    // lista filtra por instance
    const list = await agentList('record', 'inst1');
    expect(list).toHaveLength(1);
    expect(list[0].id).toBe(rec.id);
    // isolation
    expect(await agentList('record', 'inst2')).toHaveLength(0);
  });

  it('agentSearch combina memoria + records', async () => {
    await agentCreate('record', { name: 'BuscarZapato' } as any, 'inst1');
    const res = await agentSearch('record', 'zapato', 'inst1');
  ...
  it('agentDelete borra y genera memoria delete', async () => {
    const rec: any = await agentCreate('record', { name: 'Borrar' } as any, 'inst1');
    expect(await agentDelete('record', rec.id, 'inst1')).toBe(true);
    expect(await agentGet('record', rec.id, 'inst1')).toBeNull();
  });
